...
#       cat("Starting targeting step for (", order, ", ", delta0, ")\n")
      ...
      epsilon <- sum((((L1 - Q1d_bar_0n) * H_delta) * (A0 == d0(L0)))[training_set_indices]) /
        sum((H_delta^2 * (A0 == d0(L0)))[training_set_indices])
      candidate_Q1d_bar_n <- Q1d_bar_0n + epsilon * H_delta_setting_A_to_d
...
cl <- makeCluster(getOption("cl.cores", 36), outfile = "C-TMLE_multi_orders_outfile")
