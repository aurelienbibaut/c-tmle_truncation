...
n_full <- 1e7
n_subsample <- floor(10^6.5)
estimation_tasks <- expand.grid(eta = etas, candidate_rate = candidate_rates)
jobs <- 1:nrow(estimation_tasks)

# Define pivot function
compute_pivot <- function(data, indices, delta, Delta, eta, n){
  replicate.data <- data[indices, ]
  
  # Compute finite differnece
  TMLE_delta_plus_Delta <- TMLE_EY1_speedglm(replicate.data, delta + Delta)
  TMLE_delta <- TMLE_EY1_speedglm(replicate.data, delta)
  fin_diff <- (TMLE_delta_plus_Delta$Psi_n - TMLE_delta$Psi_n) / Delta
    
  (abs(fin_diff) * delta / sqrt(TMLE_delta$var_IC))^eta * sqrt(n)
}
...
# Subsamples indices
nb_subsamples <- 10
subsamples.indices <- t(replicate(10, sample(1:n_full, n_subsample, F)))
