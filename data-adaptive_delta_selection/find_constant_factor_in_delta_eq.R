...
finite_diff_estimator <- function(observed_data, delta, n){
  ...
  Psi_n_delta_plus_Delta <- TMLE_EY1_bootstrap(observed_data, delta + Delta)$Psi_n
...
results <- foreach(i=1:nrow(jobs), .combine = rbind) %dopar% {
  ...
  fin_diff_result <- finite_diff_estimator(observed_data, delta, n)
  LHS <- abs(fin_diff_result$fin_diff * delta^2)^eta * n
  
  c(n, rate, eta, LHS, fin_diff_result$p_val)
}

colnames(results) <- c("n", "base_rate", "eta", "LHS")
# Plot the results
...
for(i in 1:length(etas)){
  LHS_plots[[i]] <- ggplot(data = subset(results_df, eta == etas[i]), aes(x = log(n)/log(10), y = log(LHS), colour = base_rate)) +
    geom_line() + ggtitle(substitute(list(eta) == list(x),
                                     list(x = etas[i])))
}
