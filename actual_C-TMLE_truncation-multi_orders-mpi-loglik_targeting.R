...
C_TMLE_truncation <- function(observed_data, d0, orders, delta0s, Q_misspecified = F, 
  ...
  # Compute the C-TMLEs for all the couples (order, delta0) and pick
  # the one that has the best cross validated loss for \bar{Q}
  test_set_indices <- sample(1:n, floor(n/5), replace = F)
  training_set_indices <- setdiff(1:n, test_set_indices)
  # The best_tmle_* quantities are associated to the the C-TMLEs of
  # target parameters, by opposition to the quantities associated
  # to the iterative targeting steps within each C-TMLE
  loss_untargeted_Q <- mean(((L1 - Q1d_bar_0n)^2 * (A0 == d0(L0)))[test_set_indices])
  best_tmle_loss <- loss_untargeted_Q
  best_tmle_Q1d_bar_n <- Q1d_bar_0n
  best_tmle_indices <- list(order = -1, delta0 = 0) #(-1,0) stands for no targeting
  
  for(order in orders){
    for(delta0 in delta0s){
      # This iteration computes the C-TMLE of Psi(order, delta0)
      
      # Define the sequence of delta0s that index the clever covariates to
      # potentially include in refining the initial estimate of \bar{Q}.  
      # These are the delta0s equal to or bigger than delta0
      deltas_potential_refiners <- delta0s[which(delta0s >= delta0)]
      current_loss <- loss_untargeted_Q
      current_Q1d_bar_n <- Q1d_bar_0n
      
      while(length(deltas_used) < length(deltas_potential_refiners)){
      
        best_refining_loss <- current_loss
        best_refining_Q1d_bar_n <- current_Q1d_bar_n
        
        for(delta in setdiff(deltas_potential_refiners, deltas_used)){
          # Define test set and training set
          test_set_indices <- sample(1:n, floor(n/5), replace = F)
          training_set_indices <- setdiff(1:n, test_set_indices)
      
          # Compute a_delta0 as defined in write-up
          result_compute_a_delta0 <- compute_a_delta0(delta, order = order, n_points, diff_step, verbose=F)
          a_delta0 <- result_compute_a_delta0$a_delta0
          deltas <- result_compute_a_delta0$deltas
      
          # Compute clever covariate
          gn0_delta0 <- g_to_g_delta(delta, gn0)
          ...
          refining_candidate_Q1d_bar_n <- expit(logit(Q1d_bar_0n) + epsilon * H_delta_setting_A_to_d)
      
          # Compute loss of the candidate on training set. Update best loss and best candidate
          # if needed
          refining_candidate_loss <- mean(((L1 - candidate_Q1d_bar_n)^2 * (A0 == d0(L0)))[test_set_indices])
          
          # Now check if this loss is the best achieved so far in the loop over the remaining 
          # potential refiners
          if(refining_candidate_loss < best_refining_loss){
            best_refining_loss <- refining_candidate_loss
            best_refined_Q1d_bar_n <- refining_candidate_Q1d_bar_n
            delta_best_refiner <- delta
          }
        }
        
        # If the best refinement obtained with the loop over remaining clever covariates
        # yields an improvement, keep it, otherwise stop refining.
        if(best_refining_loss < current_loss){
          deltas_used <- c(deltas_used, delta_best_refiner)
          current_loss <- best_refining_loss
          current_Q1d_bar_n <- best_refined_Q1d_bar_n
        }else{
          break
        }
      }
      # End of targeting steps of C-TMLE of Psi(order, delta0)
      # WRONG, has to be a CTMLE for each order, wait dunno, figure
      # it out tomorrow.
